...
function determinePaneXLogical(chart: IChartApi, x: number | null): Logical | null {
  if (x === null) return null;
  return chart.timeScale().coordinateToLogical(x);
}

function determineYPosition(chartElement: HTMLDivElement, clientY: number): Coordinate {
...
export class MultiTouchChartEvents {
  ...
  _mouseDown(event: MouseEvent) {
    ...
    this._mouseState.drawing =
      this._mouseState.startLogical !== null && this._mouseState.startCoordinate !== null;
  }
  _mouseUp() {
    this._mouseState.drawing = false;
  }
