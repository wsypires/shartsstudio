...
function extractPrice(dataPoint: SeriesDataItemTypeMap[SeriesType]): number | undefined {
  if ((dataPoint as BarData).close !== undefined) return (dataPoint as BarData).close;
  if ((dataPoint as LineData).value !== undefined) return (dataPoint as LineData).value;
  return undefined;
}
