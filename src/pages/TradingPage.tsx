...
import { useSignalAlertsEngine } from "../services/alerts/useSignalAlertsEngine.ts";
import { SignalAlertManagerDialog } from "../components/alerts/SignalAlertManagerDialog.tsx";
import { SignalsPanel } from "../components/alerts/SignalsPanel.tsx";
import { SignalExecutionBanner } from "../components/alerts/SignalExecutionBanner.tsx";
...
export function TradingPage() {
  ...
  const [rightPanel, setRightPanel] = useState<
    "order" | "dom" | "watchlist" | "news" | "ai-trader" | "tv-analysis" | "signals"
  >("order");
  const [showRightPanel, setShowRightPanel] = useState(true);

  // ── Advanced Signal Alerts State ──
  const [isSignalAlertModalOpen, setIsSignalAlertModalOpen] = useState(false);
  const [signalAlertInitialPrice, setSignalAlertInitialPrice] = useState<number | null>(null);

  const handleOpenSignalAlertModal = useCallback((price?: number) => {
    if (typeof price === "number" && Number.isFinite(price)) {
      setSignalAlertInitialPrice(price);
    } else {
      setSignalAlertInitialPrice(null);
    }
    setIsSignalAlertModalOpen(true);
  }, []);
  ...
  const currentPrice = tick ? (tick.bid + tick.ask) / 2 : 0;
  const pipDigits = useMemo(
    () => getPipDigits(symbolInfo, selectedSymbol),
    [symbolInfo, selectedSymbol],
  );

  // Signal Alerts real-time background evaluation engine
  useSignalAlertsEngine({
    selectedSymbol,
    timeframe,
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <ChartToolbar
        ...
        onOpenSignalAlertDialog={handleOpenSignalAlertModal}
      ...
      <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
        ...
        <div className="flex flex-col flex-1 min-w-0">
          ...
          {/* Bottom Panel (Positions / Orders / Journal / Calendar / News / Signals) */}
          <BottomPanel
            tab={bottomTab}
            ...
            {rightPanel === "signals" && (
              <div className="flex-1 overflow-hidden flex flex-col">
                <SignalsPanel
                  selectedSymbol={selectedSymbol}
                  currentPrice={currentPrice}
                  onOpenCreateDialog={handleOpenSignalAlertModal}
                />
              </div>
            )}
            {rightPanel === "news" && (
      ...
      {/* ── Signal Execution Floating Banner ──── */}
      <SignalExecutionBanner
        accountId={activeAccountId}
        onOrderSuccess={() => {
          playTradeSound();
          handleFirstTrade();
      ...
      {/* ── Signal Alerts & Automation Rule Manager Dialog ──── */}
      <SignalAlertManagerDialog
        isOpen={isSignalAlertModalOpen}
        onClose={() => setIsSignalAlertModalOpen(false)}
        selectedSymbol={selectedSymbol}
        currentPrice={currentPrice}
        accountId={activeAccountId}
        initialPrice={signalAlertInitialPrice}
      />
