...
import { useSignalAlertsOverlay } from "./useSignalAlertsOverlay.ts";
import { ChartAlertsOverlayBadge } from "../../components/alerts/ChartAlertsOverlayBadge.tsx";
import {
  formatCountdown,
  getCandleBucketTime,
  getMinMove,
...
export interface ChartPanelProps {
  ...
  /** Open the advanced signal alert rule builder dialog with target price */
...
export function ChartPanel({
  ...
  const handleAddAlert = useCallback(
    (price: number) => {
      ...
      if (onOpenSignalAlertDialog) {
        onOpenSignalAlertDialog(rounded);
      } else {
        toast.info("Alert set", `${selectedSymbol} at ${rounded}`);
      }
    },
    [onAddDrawing, pipDigits, timeframe, selectedSymbol, onOpenSignalAlertDialog],
  );

  useIndicators(chartRef, candleSeriesRef, chartData, activeIndicators, isDark);

  useSignalAlertsOverlay({
    chartRef,
    candleSeriesRef,
    chartData,
    selectedSymbol,
    timeframe,
    chartEpoch,
    currentPrice: tick ? (tick.bid + tick.ask) / 2 : undefined,
    pipDigits,
  });
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {/* Active Signal Alerts Floating HUD on Chart */}
      <ChartAlertsOverlayBadge
        selectedSymbol={selectedSymbol}
        timeframe={timeframe}
        currentPrice={tick ? (tick.bid + tick.ask) / 2 : 0}
        accountId={accountId ?? null}
        onOpenManager={(targetPrice) => onOpenSignalAlertDialog?.(targetPrice)}
      />
